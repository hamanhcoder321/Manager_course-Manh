...
if (isGet()) { // xảy ra phương thức get
    ...
    if (!empty($keyword)) { // nếu keyword có dlieu ko rỗng
        ...
        $chuoiWhere .= " name LIKE '%$keyword%' OR slug LIKE '%$keyword%' "; // tìm kiếm theo tên và so sánh chuỗi dlieu
    }
}


// xử lý phân trang
$maxData = getRows("SELECT id FROM course_category"); // tổng dlieu bảng course
...
$getDetailCate = getAll("SELECT * FROM course_category $chuoiWhere
ORDER BY created_at DESC
LIMIT $offset, $perPage
");


if(!empty($keyword)){ // nếu keyword có dlieu
    $maxData = getRows("SELECT * FROM course_category $chuoiWhere");// đặt lại max data
    $maxPage = ceil($maxData/$perPage); // tính max
}
            ...
            <div class="container grid-user">
                <div class="container-fuild">
                    <div class="plus-search">
                        ...
                        <a href="?module=course_category&action=add" class="btn btn-success mb-3"><i
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered text-center">
                            ...
                            <tbody>
                                ...
                                foreach ($getDetailCate as $key => $item): // key là id, item dlieu
                                    ?>
                                    <tr>
                                        ...
                                        <td><a href="?module=course_category&action=edit&id=<?php echo $item['id']; ?>"
                                            ...
                                            <a href="?module=course_category&action=delete&id=<?php echo $item['id']; ?>"
